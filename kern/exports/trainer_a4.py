...
def _make_qr_image_bytes(payload: str) -> bytes:
    qr = qrcode.QRCode(
        version=None,
...
def export_trainer_cards(
    ...
    *,
    ...
    # Wir verwenden outer_m als Karten-Rand; m bleibt als "Page Safe" ungenutzt, da outer_m klarer ist.
    ...
    def card_xy(index_in_page: int) -> (float, float):
    ...
    while start < total:
        ...
        for i_in_page, it in enumerate(batch):
            ...
            slug = _choose_icon_slug(subject, word, legacy_icon_slug=(legacy.get("icon_slug") if isinstance(legacy, dict) else None))
        ...
        for i_in_page, it in enumerate(batch):
            ...
            # Satz in 2-3 Zeilen umbrechen (minimalistisch, offline)
            max_chars = 42
            s = sentence.strip()
            lines: List[str] = []
            while len(s) > max_chars and len(lines) < 3:
                cut = s.rfind(" ", 0, max_chars)
                if cut <= 0:
                    cut = max_chars
                lines.append(s[:cut].strip())
                s = s[cut:].strip()
            if s and len(lines) < 3:
                lines.append(s)
