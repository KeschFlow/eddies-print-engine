...
BLEED = 0.125 * inch          # KDP: bleed extends 0.125" beyond trim
INTERIOR_SAFE = 0.375 * inch  # KDP: outside margin >= 0.375" (with bleed, lower page counts)
...
def build_cover_wrap(child_name: str, page_count: int = PAGES_DEFAULT, paper: str = "B&W – White") -> bytes:
    ...
    # Optional stability minimum (only for drawing)
    spine_w_draw = max(spine_w, 0.06 * inch)

    cover_w = (2 * TRIM_SIZE) + spine_w + (2 * BLEED)   # keep math exact
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # SPINE (fill exact spine area; draw icon centered using draw width)
        c.setFillColor(colors.black)
        ...
        if page_count >= SPINE_TEXT_MIN_PAGES:
            ...
            # Use draw width if spine is tiny
            _draw_eddie_mark(c, spine_x0 + spine_w / 2, BLEED + TRIM_SIZE / 2, r=min(0.18 * inch, spine_w_draw * 0.35))
...
st.markdown('<div class="subtitle">Erstelle dein komplettes KDP-Upload-Set: <b>Interior PDF</b> + <b>Cover-Wrap PDF</b></div>', unsafe_allow_html=True)
...
with st.container():
    ...
    paper = st.selectbox("Papier / Ink (Spine-Berechnung)", options=list(PAPER_FACTORS.keys()), index=0)
    ...
    st.caption("Wenn du weniger hochlädst, wird automatisch wiederholt (deterministisch).")
...
if st.session_state.interior_pdf and st.session_state.cover_pdf:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown("### 💜 Unterstütze Eddie’s Welt (optional)")
    amount = st.number_input("Betrag (€)", min_value=0, value=PWYW_SUGGESTION, step=1)
    st.caption("PWYW ist aktuell optional. (Stripe/PayPal kann als nächster Schritt live gehen.)")
