# =========================================================
# app.py — E. P. E. Eddie's Print Engine — v5.10.1 FINAL (GitHub Drop)
#
# QA FIXPACK & EXTREME HARDENING:
# - PREFLIGHT: SAFE H (odd/even) correctly displayed
# - UI STABILITY: _uploads_sig() soft-fails on oversized files
# - PREFLIGHT: Barcode Zone text updated & exact dimensions added
# - SECURITY: Max Total Upload Bytes Cap (160MB) added
# - SECURITY: OpenCV decode shape pixel guard (25MP limit)
# - COVER PREFLIGHT: Added Facts Strip with absolute PDF coordinates
# - COVER SAFE BOX: Added dashed safe boxes (0.25") on back & front
# =========================================================
...
Image.MAX_IMAGE_PIXELS = 25_000_000  # ~25MP, schützt vor Decompression Bombs
...
def _get_zone_for_hour(hour: int) -> ZoneStub:
    if qd and hasattr(qd, "get_zone_for_hour"):
        try:
            ...
            return ZoneStub(getattr(z, "name", "Zone"), getattr(z, "icon", "🟣"), getattr(z, "quest_type", "Quest"), getattr(z, "atmosphere", ""))
        except Exception:
            pass
...
def _build_full_quest_bank():
    ...
    for h in range(24):
        ...
        for i in range(10):
            ...
            if i % 3 == 0: thinking = "Finde {tri} Dreiecke, {sq} Quadrate und {st_} Sterne."
            elif i % 3 == 1: thinking = "Spüre insgesamt {total} versteckte Formen auf (△, □, ★)."
            else: thinking = "Suche: {tri}x Dreieck, {sq}x Quadrat, {st_}x Stern."
...
def build_book_schedule(seed: int, start_hour: int, count: int) -> dict:
    ...
    for i in range(count):
        ...
        if not candidates: candidates = [q for q in QUEST_RESERVE if q["id"] not in used_ids]
        if not candidates: candidates = QUEST_RESERVE
...
PAPER_FACTORS = {"Schwarzweiß – Weiß": 0.002252, "Schwarzweiß – Creme": 0.0025, "Farbe – Weiß (Standard)": 0.002252}
KDP_PAGES_FIXED = 26  # 24 Missions + Intro + Outro
SPINE_TEXT_MIN_PAGES = 79

MAX_SKETCH_CACHE = 256
MAX_WASH_CACHE = 64

BUILD_TAG = "v5.10.1-final"
...
def _imprint_nonce(c: canvas.Canvas, build_nonce: str) -> None:
    try:
        c.saveState(); c.setFillColor(colors.white); c.setFont("Helvetica", 1)
        c.drawString(-1000, -1000, f"nonce:{build_nonce}"); c.restoreState()
    except Exception: pass

@dataclass(frozen=True)
class PageBox:
    trim_w: float; trim_h: float; bleed: float; full_w: float; full_h: float
...
def safe_margins_for_page(pages: int, kdp: bool, page_index_0: int, pb: PageBox) -> Tuple[float, float, float]:
    if not kdp: return SAFE_INTERIOR, SAFE_INTERIOR, SAFE_INTERIOR
...
def _try_register_fonts() -> Dict[str, str]:
    n, b = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    if os.path.exists(n):
...
def _set_font(c: canvas.Canvas, bold: bool, size: int) -> float:
    c.setFont(FONTS["bold"] if bold else FONTS["normal"], size); return size * 1.22
            ...
            if not fits(w):
                ...
                for ch in w:
                    if fits(chunk + ch): chunk += ch
                        ...
                        if chunk: lines.append(chunk); chunk = ch
                cur = chunk
            else: cur = w
    if cur: lines.append(cur)
    return lines
...
def _stable_seed(s: str) -> int: return int.from_bytes(hashlib.sha256(s.encode("utf-8")).digest()[:8], "big")
...
def _draw_cover_preflight_facts_strip(c, *, cw, ch, trim, bleed, spine_w, build_nonce, paper, back_x, spine_x, front_x, trim_y, barcode_x, barcode_y, barcode_w, barcode_h):
    pad, x0 = 0.20 * inch, back_x + 0.20 * inch
    strip_w, strip_h = min(trim - 2 * pad, 3.55 * inch), 1.55 * inch
    y_bottom = (trim_y + trim - strip_h - pad)
    c.saveState(); c.setFillColor(colors.white); c.setStrokeColor(colors.HexColor("#111111")); c.setLineWidth(0.8)
    ...
    line, yy = 0.14 * inch, y_bottom + strip_h - 0.60 * inch
    def row(label, v_in, v_pt):
        nonlocal yy
...
def _draw_preflight_facts_page(c: canvas.Canvas, pb: PageBox, total_pages: int, kdp: bool, paper: str, build_nonce: str) -> None:
    ...
    bleed_in, full_w_in, full_h_in = pb.bleed / inch, pb.full_w / inch, pb.full_h / inch
    ...
    safe_w_odd, safe_h_odd = pb.full_w - (safe_l_odd + safe_r_odd), pb.full_h - (2 * safe_tb_odd)
    safe_w_even, safe_h_even = pb.full_w - (safe_l_even + safe_r_even), pb.full_h - (2 * safe_tb_even)
    def _as_in(x_pt: float) -> float: return x_pt / inch
    spine_in = max(float(total_pages) * float(PAPER_FACTORS.get(paper, 0.002252)), 0.001)

    c.saveState(); c.setFillColor(colors.white); c.rect(0, 0, pb.full_w, pb.full_h, fill=1, stroke=0)
    c.setFillColor(INK_BLACK); _set_font(c, True, 24); c.drawCentredString(pb.full_w / 2, pb.full_h - 1.0 * inch, "KDP PREFLIGHT — NO FEELINGS, JUST FACTS")
    ...
    c.setFillColor(INK_BLACK); _set_font(c, True, 12); c.drawString(left, top, "PAGE GEOMETRY"); _set_font(c, False, 11)
...
@dataclass
class ShapeSpec:
    kind: str; cx: float; cy: float; size: float; rot: float
...
def _generate_shapes(pb: PageBox, sl: float, sr: float, stb: float, pre_reader: bool, seed: int) -> List[ShapeSpec]:
    ...
    min_y, max_y = stb + card_h + pad, pb.full_h - stb - 0.75 * inch - pad
    if max_x <= min_x or max_y <= min_y: return []
    shapes = []
    for _ in range(int(rng.integers(3, 8))):
        shapes.append(ShapeSpec(str(rng.choice(["triangle", "square", "star"])), float(rng.uniform(min_x, max_x)), float(rng.uniform(min_y, max_y)), float(rng.uniform(0.28, 0.58)) * inch, float(rng.uniform(0, 360))))
    return shapes

def _draw_shapes(c: canvas.Canvas, shapes: List[ShapeSpec]):
    if not shapes: return
    c.saveState(); c.setStrokeColor(INK_BLACK); c.setLineWidth(2.2); c.setFillColor(colors.white)
    for s in shapes:
        c.saveState(); c.translate(s.cx, s.cy); c.rotate(s.rot)
        if s.kind == "triangle":
            p = c.beginPath(); p.moveTo(0, s.size / 2); p.lineTo(-s.size / 2, -s.size / 2); p.lineTo(s.size / 2, -s.size / 2); p.close(); c.drawPath(p, fill=1, stroke=1)
        elif s.kind == "square":
            c.rect(-s.size / 2, -s.size / 2, s.size, s.size, fill=1, stroke=1)
        else:
            p = c.beginPath(); r_out, r_in = s.size / 2, (s.size / 2) / 2.5
            for i in range(10):
                r = r_out if i % 2 == 0 else r_in; theta = i * (math.pi / 5) - (math.pi / 2); x, y = r * math.cos(theta), r * math.sin(theta)
                if i == 0: p.moveTo(x, y)
                else: p.lineTo(x, y)
...
def _get_lru(name: str, max_items: int) -> "OrderedDict":
    ...
    if not isinstance(od, OrderedDict): od, st.session_state[name] = OrderedDict(), OrderedDict()
    st.session_state[f"{name}__max"] = max_items
    return od

def _lru_put(od: "OrderedDict", key, value, max_items: int):
    od[key] = value; od.move_to_end(key)
    while len(od) > max_items: od.popitem(last=False)
...
def _read_upload_bytes(up) -> bytes:
    ...
    if len(b) > MAX_UPLOAD_BYTES: raise ValueError(f"Upload zu groß (max 12MB pro Bild): {getattr(up, 'name', 'Unbekannt')}")
    return b

def _wash_bytes(raw: bytes) -> bytes:
    if not raw: raise ValueError("empty upload")
    if hasattr(iw, "wash_image_bytes"): return bytes(iw.wash_image_bytes(raw))
    if hasattr(iw, "wash_bytes"): return bytes(iw.wash_bytes(raw))
    raise RuntimeError("image_wash logic missing")

def _wash_upload_to_bytes(up) -> bytes:
    raw = _read_upload_bytes(up); h = hashlib.sha256(raw).hexdigest()
    wc = _get_lru("wash_cache", MAX_WASH_CACHE)
    if h in wc: wc.move_to_end(h); return wc[h]
    w = _wash_bytes(raw); _lru_put(wc, h, w, MAX_WASH_CACHE); return w

def _sketch_compute(img_bytes: bytes, target_w: int, target_h: int) -> bytes:
    arr = cv2.imdecode(np.frombuffer(img_bytes, np.uint8), cv2.IMREAD_COLOR)
    if arr is None: raise RuntimeError("OpenCV decode failed")
    h_arr, w_arr = arr.shape[:2]
    if (w_arr * h_arr) > 25_000_000: raise ValueError("Bildauflösung zu groß (max ~25MP).")
    ...
    pil = Image.fromarray(norm).convert("L"); sw, sh = pil.size; s = min(sw, sh)
    ...
    outv = out.getvalue()
    del arr, gray, blurred, sketch, norm, pil; gc.collect()
    return outv
...
def _get_sketch_cached(img_bytes: bytes, target_w: int, target_h: int) -> bytes:
    ...
    out = _sketch_compute(img_bytes, target_w, target_h); _lru_put(cache, key, out, MAX_SKETCH_CACHE); return out

# =========================================================
# OVERLAY (QUEST CARD)
# =========================================================
def _draw_quest_overlay(c, pb, sl, sr, stb, hour, mission: Mission, debug, pre_reader):
    hh = 0.75 * inch; x0 = sl; w = max(1.0, pb.full_w - sr - x0); ytb = pb.full_h - stb - hh
    zone = _get_zone_for_hour(hour); z_rgb = _get_hour_color(hour)
    c.saveState(); c.setFillColor(colors.Color(z_rgb[0], z_rgb[1], z_rgb[2])); c.setStrokeColor(INK_BLACK); c.setLineWidth(1)
    c.rect(x0, ytb, w, hh, fill=1, stroke=1)
    c.setFillColor(colors.white if sum(z_rgb[:3]) < 1.5 else INK_BLACK)
    _set_font(c, True, 14); c.drawString(x0 + 0.18 * inch, ytb + hh - 0.5 * inch, f"{_fmt_hour(hour)}  {zone.icon}  {zone.name}")
    _set_font(c, False, 10); c.drawString(x0 + 0.18 * inch, ytb + 0.18 * inch, f"{zone.quest_type} • {zone.atmosphere}")
    cy = stb; max_ch = ytb - cy - 0.15 * inch
    if pre_reader: ch, sc = min(max_ch, 2.45 * inch), None
    else: sc = _autoscale_mission_text(mission, w, x0, 0.18 * inch, max_ch); ch = min(max_ch, max(1.85 * inch, sc["needed"]))
    c.setFillColor(colors.white); c.setStrokeColor(INK_BLACK); c.rect(x0, cy, w, ch, fill=1, stroke=1)
    yc = cy + ch - 0.18 * inch; c.setFillColor(INK_BLACK)
    if pre_reader:
        _set_font(c, True, 14); c.drawString(x0 + 0.18 * inch, yc - 10, _kid_short(mission.title, 3) or "MISSION")
        _set_font(c, True, 11); c.drawRightString(x0 + w - 0.18 * inch, yc - 10, f"+{int(mission.xp)} XP")
        sy = yc - 0.50 * inch
        _icon_run(c, x0 + 0.18 * inch, sy - 0.068 * inch, 0.34 * inch); _set_font(c, False, 12); c.drawString(x0 + 0.45 * inch, sy, _kid_short(mission.movement, 3) or "Bewegen!")
        _icon_brain(c, x0 + 0.18 * inch, sy - 0.46 * inch - 0.068 * inch, 0.34 * inch); _set_font(c, False, 12); c.drawString(x0 + 0.45 * inch, sy - 0.46 * inch, mission.thinking)
        _icon_check(c, x0 + 0.18 * inch, sy - 0.92 * inch - 0.068 * inch, 0.34 * inch); _set_font(c, False, 12); c.drawString(x0 + 0.45 * inch, sy - 0.92 * inch, _kid_short(mission.proof, 4) or "Haken!")
        _set_font(c, False, 8); c.setFillColor(INK_GRAY_70); c.drawString(x0 + 0.18 * inch, cy + 0.12 * inch, "Eltern: kurz vorlesen – Kind macht’s nach.")
    else:
        _set_font(c, True, sc["ts"]); c.drawString(x0 + 0.18 * inch, yc - sc["tl"] + 2, f"MISSION: {mission.title}")
        _set_font(c, True, max(8, sc["ts"] - 2)); c.drawRightString(x0 + w - 0.18 * inch, yc - sc["tl"] + 2, f"+{int(mission.xp)} XP")
        yt = yc - sc["tl"] - 0.10 * inch
        _set_font(c, True, sc["ls"]); c.drawString(x0 + 0.18 * inch, yt - sc["ll"] + 2, "BEWEGUNG:"); _set_font(c, False, sc["bs"])
        yy = yt - sc["ll"] + 2
        for l in sc["ml"]: c.drawString(x0 + 1.05 * inch, yy, l); yy -= sc["bl"]
        yt = yy - 0.06 * inch
        _set_font(c, True, sc["ls"]); c.drawString(x0 + 0.18 * inch, yt - sc["ll"] + 2, "SUCHEN:"); _set_font(c, False, sc["bs"])
        yy = yt - sc["ll"] + 2
        ...
        _set_font(c, True, sc["ls"]); c.drawString(x0 + 0.43 * inch, cy + 0.20 * inch, "PROOF:"); _set_font(c, False, sc["bs"])
        if mission.proof: c.drawString(x0 + 1.03 * inch, cy + 0.20 * inch, _fit_lines(_wrap_text_hard(mission.proof, FONTS["normal"], sc["bs"], w - 1.5 * inch), 1)[0])
    if debug: _draw_kdp_debug_guides(c, pb, sl, sr, stb)
    c.restoreState()
...
def _cover_collage_png(uploads, size_px: int, seed: int) -> Optional[bytes]:
    ...
    gap = max(10, size_px // 120); cell = (size_px - gap * 3) // 2
    canvas_img = Image.new("RGB", (size_px, size_px), (255, 255, 255)); k = 0
    for r in range(2):
        for c_ in range(2):
            if k >= len(files): break
            try: sk = _sketch_compute(_wash_upload_to_bytes(files[k]), cell, cell); tile = Image.open(io.BytesIO(sk)).convert("RGB")
            except: tile = Image.new("RGB", (cell, cell), (255, 255, 255))
            canvas_img.paste(tile, (gap + c_ * (cell + gap), gap + r * (cell + gap)))
            k += 1; del tile; gc.collect()
    ImageDraw.Draw(canvas_img).rectangle([0, 0, size_px - 1, size_px - 1], outline=(0, 0, 0), width=max(2, size_px // 250))
    out = io.BytesIO(); canvas_img.save(out, format="PNG", optimize=True)
    return out.getvalue()
...
def build_interior(name, uploads, kdp, debug, preflight, paper, eddie, style, pre_reader, build_nonce) -> bytes:
    if not uploads: raise ValueError("Keine Uploads vorhanden. Bitte lade Fotos hoch.")
    MAX_UPLOADS = 48
    if len(uploads) > MAX_UPLOADS: raise ValueError(f"Zu viele Uploads (max {MAX_UPLOADS}).")
    MAX_TOTAL_UPLOAD_BYTES = 160 * 1024 * 1024
    total_bytes = sum(len(up.getbuffer()) for up in uploads)
    if total_bytes > MAX_TOTAL_UPLOAD_BYTES: raise ValueError(f"Uploads insgesamt zu groß (max {MAX_TOTAL_UPLOAD_BYTES // (1024*1024)}MB).")

    total, MISSION_PAGES = KDP_PAGES_FIXED, 24
    pb = page_box(TRIM, TRIM, kdp_bleed=kdp)
    final = (list(uploads) * (MISSION_PAGES // len(uploads) + 1))[:MISSION_PAGES]
    buf = io.BytesIO(); c = canvas.Canvas(buf, pagesize=(pb.full_w, pb.full_h))
    c.setTitle(f"{APP_TITLE} — Interior"); c.setAuthor("Eddies World"); c.setSubject(f"nonce={build_nonce}")
    schedule = build_book_schedule(_stable_seed(build_nonce), start_hour=6, count=MISSION_PAGES)
    seed_base, nonce_seed, current_page_idx = _stable_seed(name), _stable_seed(build_nonce), 0
    ...
    if preflight:
        ...
        if debug: _draw_kdp_debug_guides(c, pb, sl, sr, stb)
        _imprint_nonce(c, build_nonce); c.showPage(); current_page_idx += 1
    else:
        c.setFillColor(colors.white); c.rect(0, 0, pb.full_w, pb.full_h, fill=1, stroke=0)
        gen = _name_genitive(name); c.setFillColor(INK_BLACK)
        _set_font(c, True, 34); c.drawCentredString(pb.full_w / 2, pb.full_h - stb - 1.90 * inch, f"{gen} Abenteuerbuch")
        _set_font(c, False, 14); c.drawCentredString(pb.full_w / 2, pb.full_h - stb - 2.35 * inch, "Erstellt mit")
        _set_font(c, True, 18); c.drawCentredString(pb.full_w / 2, pb.full_h - stb - 2.70 * inch, "E. P. E.")
        _set_font(c, False, 14); c.drawCentredString(pb.full_w / 2, pb.full_h - stb - 3.00 * inch, "Eddie's Print Engine")
        _draw_eddie(c, pb.full_w / 2, pb.full_h / 2, 1.20 * inch, style=style)
        c.setFillColor(INK_GRAY_70); _set_font(c, False, 13); c.drawCentredString(pb.full_w / 2, stb + 0.75 * inch, "24 Stunden • 24 Mini-Quests • Haken setzen")
        if debug: _draw_kdp_debug_guides(c, pb, sl, sr, stb)
        _imprint_nonce(c, build_nonce); c.showPage(); current_page_idx += 1
    ...
    for i, up in enumerate(final):
        ...
        hour = (6 + i) % 24; seed = int(seed_base ^ nonce_seed ^ (i << 1) ^ hour) & 0xFFFFFFFF
        shapes = _generate_shapes(pb, sl, sr, stb, bool(pre_reader), seed)
        _draw_shapes(c, shapes)
        tri, sq, st_, t_shapes = sum(1 for s in shapes if s.kind == "triangle"), sum(1 for s in shapes if s.kind == "square"), sum(1 for s in shapes if s.kind == "star"), len(shapes)
        q_data = schedule[hour]
        if pre_reader: m_think, m_move, m_proof = f"{tri} △   {sq} □   {st_} ★", _kid_short(q_data["movement"], 3), "Haken!"
        else: m_think, m_move, m_proof = q_data["thinking"].format(tri=tri, sq=sq, st_=st_, total=t_shapes), q_data["movement"], q_data["proof"]
        mission = Mission(title=q_data["title"], xp=q_data["xp"], movement=m_move, thinking=m_think, proof=m_proof)
        _draw_quest_overlay(c, pb, sl, sr, stb, hour, mission, debug, pre_reader)
        if eddie: _draw_eddie(c, pb.full_w - sr - 0.18 * inch, stb + 0.18 * inch, 0.18 * inch, style=style)
        _imprint_nonce(c, build_nonce); c.showPage(); current_page_idx += 1; gc.collect()

    # OUTRO PAGE
    sl, sr, stb = safe_margins_for_page(total, kdp, current_page_idx, pb)
    c.setFillColor(colors.white); c.rect(0, 0, pb.full_w, pb.full_h, fill=1, stroke=0)
    _draw_eddie(c, pb.full_w / 2, pb.full_h / 2 + 0.6 * inch, 1.5 * inch, style=style)
    c.setFillColor(INK_BLACK); _set_font(c, True, 30); c.drawCentredString(pb.full_w / 2, pb.full_h / 2 - 1.5 * inch, "Quest abgeschlossen!")
    if debug: _draw_kdp_debug_guides(c, pb, sl, sr, stb)
    _imprint_nonce(c, build_nonce); c.showPage()
    c.save(); buf.seek(0)
    return buf.getvalue()
...
def build_cover(name, paper, uploads, style, build_nonce, debug, preflight) -> bytes:
    ...
    buf = io.BytesIO(); c = canvas.Canvas(buf, pagesize=(cw, ch))
    c.setTitle(f"{APP_TITLE} — Cover"); c.setAuthor("Eddies World")
    if build_nonce: c.setSubject(f"nonce={build_nonce}")
    ...
    back_x, spine_x, front_x, trim_y = BLEED, BLEED + TRIM, BLEED + TRIM + sw, BLEED
    if KDP_PAGES_FIXED >= SPINE_TEXT_MIN_PAGES:
        c.saveState(); c.setFillColor(colors.white); _set_font(c, True, 10); c.translate(BLEED + TRIM + sw / 2, BLEED + TRIM / 2); c.rotate(90); c.drawCentredString(0, -4, f"{_name_genitive(name)} ABENTEUERBUCH".upper()); c.restoreState()

    bx = back_x
    c.setFillColor(colors.white); c.rect(bx, BLEED, TRIM, TRIM, fill=1, stroke=0)
    _draw_eddie(c, bx + TRIM * 0.12, BLEED + TRIM * 0.86, TRIM * 0.06, style=style)
    c.setFillColor(INK_GRAY_70); _set_font(c, False, 12); c.drawString(bx + TRIM * 0.12, BLEED + TRIM * 0.12, "24 Missionen • 24 Stunden • KDP-ready")
    ...
    if collage:
        ...
        c.setFillColor(colors.white); c.setStrokeColor(INK_BLACK); c.setLineWidth(1)
        c.roundRect(fx + TRIM * 0.10, BLEED + TRIM * 0.74, TRIM * 0.80, TRIM * 0.20, TRIM * 0.04, fill=1, stroke=1)

    gen = _name_genitive(name)
    c.setFillColor(INK_BLACK); _set_font(c, True, 30); c.drawCentredString(fx + TRIM / 2, BLEED + TRIM * 0.88, f"{gen}")
    _set_font(c, True, 24); c.drawCentredString(fx + TRIM / 2, BLEED + TRIM * 0.82, "ABENTEUERBUCH")
    _set_font(c, False, 11); c.setFillColor(INK_GRAY_70); c.drawCentredString(fx + TRIM / 2, BLEED + TRIM * 0.77, "Erstellt mit E. P. E. — Eddie's Print Engine")
    _draw_eddie(c, fx + TRIM / 2, BLEED + TRIM * 0.60, TRIM * 0.14, style=style)

    if debug or preflight:
        bc_w, bc_h = 2.0 * inch, 1.2 * inch
        bc_x, bc_y = BLEED + TRIM - 0.25 * inch - bc_w, BLEED + 0.25 * inch
        c.saveState(); c.setStrokeColor(colors.red); c.setLineWidth(1); c.setDash(4, 4); c.rect(bc_x, bc_y, bc_w, bc_h, stroke=1, fill=0); c.setFillColor(colors.red); _set_font(c, True, 10); c.drawCentredString(bc_x + bc_w / 2, bc_y + 0.65 * inch, "KDP BARCODE ZONE"); _set_font(c, False, 8); c.drawCentredString(bc_x + bc_w / 2, bc_y + 0.45 * inch, "2.0\" × 1.2\""); c.restoreState()
        _draw_cover_preflight_facts_strip(c, cw=cw, ch=ch, trim=TRIM, bleed=BLEED, spine_w=sw, build_nonce=str(build_nonce), paper=str(paper), back_x=back_x, spine_x=spine_x, front_x=front_x, trim_y=trim_y, barcode_x=bc_x, barcode_y=bc_y, barcode_w=bc_w, barcode_h=bc_h)
        safe_margin = 0.25 * inch
        def _safe_box(x0, y0, w, h, label):
            c.saveState(); c.setStrokeColor(DEBUG_SAFE_COLOR); c.setLineWidth(1); c.setDash(3, 3); c.rect(x0, y0, w, h, stroke=1, fill=0); c.setDash(); c.setFillColor(DEBUG_SAFE_COLOR); _set_font(c, True, 8); c.drawString(x0 + 0.06 * inch, y0 + h + 0.06 * inch, label); _set_font(c, False, 7); c.drawString(x0 + 0.06 * inch, y0 + h - 0.18 * inch, "SAFE = 0.25\" from TRIM"); c.restoreState()
        box_w, box_h = TRIM - 2 * safe_margin, TRIM - 2 * safe_margin
    ...
    if build_nonce: _imprint_nonce(c, build_nonce)
    c.save(); buf.seek(0)
    return buf.getvalue()
...
if is_supporter: st.success("💖 **Unterstützer-Modus aktiv:** Unlimitierte Builds freigeschaltet. Danke!")
elif access_mode == "Community": st.info(f"🏫 **Community-Pass aktiv:** {builds_left} Builds in den letzten 24h verfügbar.")
elif access_mode == "Dev Mode": st.info("🧪 **Dev Mode aktiv:** Unlimitierter Zugriff (keine Stripe Secrets).")
else: st.info(f"🔓 **Open Access:** ({builds_left} kostenlose Builds in den letzten 24h übrig)")
    ...
    with st.expander("🏫 Hast du einen Community-Pass? (Kitas, Schulen, Therapeuten)"):
        ...
        if st.button("Passwort aktivieren"):
            if c_code in COMMUNITY_TOKENS:
                st.session_state["community_pass"] = c_code; st.success("Passwort akzeptiert! Seite wird neu geladen..."); time.sleep(1); st.rerun()
            else: st.error("Ungültiger Code.")

if qd is None:
    st.warning("quest_data.py fehlt – Engine läuft trotzdem (Fallback-Welt aktiv).")
    if "_QD_IMPORT_ERROR" in globals() and _QD_IMPORT_ERROR: st.code(_QD_IMPORT_ERROR, language="text")

st.session_state.setdefault("assets", None); st.session_state.setdefault("upload_sig", ""); st.session_state.setdefault("last_nonce", "")
_get_lru("sketch_cache", MAX_SKETCH_CACHE); _get_lru("wash_cache", MAX_WASH_CACHE)
    ...
    for up in (ul or []):
        try:
            ...
            if len(buf) > MAX_UPLOAD_BYTES:
                h.update(b"OVERSIZE"); h.update(getattr(up, "name", "x").encode("utf-8", "ignore")); continue
            h.update(len(buf).to_bytes(8, "little")); data = bytes(buf[:2048]) + bytes(buf[-2048:]) if len(buf) > 4096 else bytes(buf); h.update(hashlib.sha256(data).digest())
        except Exception:
            try: b = _read_upload_bytes(up)
            except ValueError:
                h.update(b"OVERSIZE"); h.update(getattr(up, "name", "x").encode("utf-8", "ignore")); continue
            h.update(len(b).to_bytes(8, "little")); h.update(hashlib.sha256(b[:2048]).digest())
    return h.hexdigest()
...
with st.container(border=True):
    ...
    age = 5
    c2.markdown("**Seitenanzahl**"); c2.info("📘 Fix 26 Seiten (KDP-Ready)")
...
if uploads: st.success(f"✅ {len(uploads)} Fotos bereit für die Engine.")
...
if not can_build_limit:
    ...
    st.markdown("**Möchtest du unbegrenzt Bücher erstellen?** Werde Unterstützer."); st.link_button("💖 Unterstützer werden (Unlimitiert)", PAYMENT_LINK, type="primary"); st.stop()

if st.button("🚀 QUESTBUCH GENERIEREN", disabled=not (uploads and name), type="primary"):
    nonce = _new_build_nonce(); st.session_state["last_nonce"] = nonce; sig = _uploads_sig(uploads)
    if st.session_state.upload_sig != sig:
        st.session_state.upload_sig = sig; st.session_state["sketch_cache"].clear(); st.session_state["wash_cache"].clear()
    with st.spinner("Engine läuft... (Waschen, Skizzieren, Shapes, Layouten)"):
        try:
            int_pdf = build_interior(name=name, uploads=uploads, kdp=bool(kdp), debug=bool(debug), preflight=bool(preflight), paper=str(paper), eddie=bool(eddie_guide), style=str(eddie_style), pre_reader=bool(pre_reader_mode), build_nonce=nonce)
            cov_pdf = build_cover(name=name, paper=str(paper), uploads=uploads, style=str(eddie_style), build_nonce=nonce, debug=bool(debug), preflight=bool(preflight))
            st.session_state.assets = {"int": int_pdf, "cov": cov_pdf, "name": name, "nonce": nonce}
            _log_build(client_ip); st.success(f"🎉 Assets bereit! (Noch {max(0, builds_left - 1)} kostenlose Builds in den letzten 24h)")
        except Exception as e:
