# =========================================================
# app.py — Eddies Questbook Edition — ULTIMATE (v4.6)
...
def _draw_quest_overlay(
    ...
    c.drawString(x0 + pad_x, y_text - sc["tl"] + 2, f"MISSION: {getattr(mission, 'title', '')}")
    _set_font(c, True, max(8, sc["ts"] - 2))
    c.drawRightString(x0 + w - pad_x, y_text - sc["tl"] + 2, f"+{getattr(mission, 'xp', 0)} XP")
