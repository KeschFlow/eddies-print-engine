...
def draw_mission_text(c: canvas.Canvas, w: float, h: float, movement: str, thinking: str, proof: str):
    # Small text block above bottom HUD (keeps your sketch page mostly untouched)
...
def build_pdf(pages: int, start_hour: int, difficulty: int, seed: int):
    ...
    for p in range(1, pages + 1):
        ...
        # 1) Render your existing sketch/activity page FIRST
        # -------------------------------------------------
        # TODO: Replace the next block with YOUR engine call.
        # Example patterns:
        # - from legacy.generate_book import draw_page
        # - from generate_book import render_activity_page
        #
        # You want something like:
        #   render_activity_page(c, page_index=p, page_width=w, page_height=h, ...)
        #
        # For now, we draw a placeholder box to show where your sketch would be.
        c.setFont("Helvetica-Bold", 16)
        c.drawString(M_L, h - M_T - HUD_TOP_H - 12*mm, f"SKETCH PAGE {p} (your engine goes here)")
        c.setLineWidth(1)
        c.rect(M_L, M_B + HUD_BOTTOM_H + 30*mm, w - M_L - M_R, h - (M_B + HUD_BOTTOM_H + 60*mm))

        # 2) Overlay HUD LAST (so it sits on top of the sketch)
        # -----------------------------------------------------
        time_str = fmt_hour(hour)
        zone_line = f"{zone.quest_type} · {zone.atmosphere}"
        draw_hud(c, w, h, zone.name, zone_line, time_str, mission.title, mission.xp)

        # Optional: show mission text (can be toggled off later)
        draw_mission_text(c, w, h, mission.movement, mission.thinking, mission.proof)

        # Page number
        c.setFont("Helvetica", 9)
        c.drawRightString(w - M_R, 6*mm, f"{p}")
...
st.caption("Paper-MMORPG: Zeit → Zone → Mission. HUD liegt über deiner bestehenden Skizzen-Engine.")
