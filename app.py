...
from engine_sketch import render_activity_page
...
def build_pdf(pages: int, start_hour: int, difficulty: int, seed: int):
    ...
    for p in range(1, pages + 1):
        ...
        render_activity_page(
            ...
            margin_left=M_L,
            margin_right=M_R,
            top_reserved=M_T + HUD_TOP_H + 6*mm,
            bottom_reserved=M_B + HUD_BOTTOM_H + 55*mm,  # space for mission text
        )
...
st.caption("Paper-MMORPG: Zeit → Zone → Mission. HUD liegt über einer echten Aktivitäts-Engine (Labyrinth + Suchauftrag).")
